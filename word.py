...
def conn():  # 连接数据库
    db = pymysql.connect(host='localhost', user='root', password='123456', db='kaoyan_word', port=3306)
    print('已连接数据库')
    return db
...
def create_table(db):  # 创建一个单词表
    ...
    sql = 'CREATE TABLE IF NOT EXISTS word (id INT(10) NOT NULL AUTO_INCREMENT,word VARCHAR(255) NOT NULL,' \
          'trans VARCHAR(255) NOT NULL,word_type VARCHAR(255) NOT NULL,pronunciation VARCHAR(255) NOT NULL,audio VARCHAR(255) NOT NULL, PRIMARY KEY (id))'
...
def get_info(word_html, type_name):  # 爬取所有的单词、发音、翻译
    ...
    audio_div = word_info.find_all('div', {'class': 'word_main_list_y'})  # 发音a内容
    ...
    for i in range(1, len(word_div)):
        ...
        audio = audio_div[i].a.get('id')  # 获取音频地址
        if len(pronunce) < 1:  # 无发音则跳过本次循环
            continue
        word_all[key] = [pronunce[0], trans, type_name,audio]  # 字典结构:字典名={'key': ['value_1','value2_',....,'value_n'],}
    print('创建数据成功')
    return word_all
...
def insert_words(word_dict, db):  # 爬取数据到数据库
    ...
    for key in word_dict:
        sql = 'INSERT INTO word(word, pronunciation, trans, word_type,audio) values(%s, %s, %s, %s, %s)'  # 构造sql语句

        try:
            print(key,'===>',word_dict[key][0],'===>',word_dict[key][1],'===>',word_dict[key][2],'===>',word_dict[key][3])
            cursor.execute(sql, (key, word_dict[key][0], word_dict[key][1], word_dict[key][2], word_dict[key][3]))
